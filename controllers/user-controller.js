...
const userController = {
   ...
   // make a request to PUT /api/users/:userId.
   // update user by id
   ...
   // make a request to DELETE /api/users/:userId.
   // delete user
   ...
   // Add a friend V1 - don't need the whole body 
   ...
   removeFriend({ params }, res) {
      ...
      .then(dbUserData => {
         if (!dbUserData) {
            res.status(404).json({ message: 'No friend found with this id to delete!' });
            return;
         }
         res.json(dbUserData);
      })
